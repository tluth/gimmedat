...
@authenticated_router.get("/files/list")
def list_files(
    auth: CognitoToken = Depends(cognito_default.auth_required),
    prefix: Optional[str] = Query(None, description="The prefix to list files from. For example, 'my/folder'"),
):
    """
    ...
    try:
        ...
        folders = [obj["Prefix"] for obj in response.get("CommonPrefixes", [])]
...
def get_upload_url(
    ...
    file_type: str = Query(..., description="The content type of the file to upload."),
    file_size: int = Query(..., description="The size of the file to upload in bytes."),
    prefix: Optional[str] = Query(None, description="The prefix to upload the file to. For example, 'my/folder'"),
):
    user_id = auth.sub
